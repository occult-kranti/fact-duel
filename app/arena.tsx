...
import Collections, { CollectionCover } from './collections';
...
import { Passport, MissionStrip } from './passport';
import Discovery from './discovery';
import Expeditions, { Clubhouse, ExpeditionCase } from './expeditions';
...
type Credentials = { roomId: string; token: string; invite?: string; profileEpoch?: string };
...
export default function Arena() {
  ...
  const { journal, save: saveFact, clear: clearJournal, persistent: storageOK } = player;
  ...
  const roomRef = useRef<any>(null),
    ...
    startMark = useRef<{ roundId: string; at: number } | null>(null),
  ...
  useEffect(() => {
    ...
    try {
      setLight(localStorage.getItem('fact-duel-online-theme') === 'light');
      setShowArt(localStorage.getItem('fact-duel-art') !== 'off');
      setSound(localStorage.getItem('fact-duel-online-sound') === 'on');
      const savedName = localStorage.getItem('fact-duel-name');
      if (savedName) setName(savedName.slice(0, 24));
      const v = Number(localStorage.getItem('fact-duel-volume') ?? '.35');
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('fact-duel-online-theme', light ? 'light' : 'dark');
      localStorage.setItem('fact-duel-online-sound', sound ? 'on' : 'off');
    } catch {}
  }, [light, sound]);
  useEffect(() => {
    try {
      localStorage.setItem('fact-duel-name', name);
      localStorage.setItem('fact-duel-volume', String(volume));
    } catch {}
  }, [name, volume]);
  ...
  const quickDuel = (mode: string, topic = 'all') => {
    ...
    const next = { ...INITIAL, mode, topic, domain: chosenTopic?.domain || 'all' };
    createDraft.current = null;
    setConfig(next);
    setJoinView(false);
    go('arena');
  ...
  return (
    <div
      className={`site-shell arcade-shell rivalry-shell journey-shell skin-${player.passport.skin} ${active ? 'active-game' : ''}`}
    >
      ...
      <header className="topbar">
        ...
        <span className="edition-label">THE KNOWLEDGE ARENA</span>
      ...
      <div className="app-frame">
        <nav className="side-nav" aria-label="Main navigation">
          {[
            ['home', 'Clubhouse', Compass],
            ['journeys', 'Expeditions', Flag],
            ['arena', 'Duels', Swords],
            ['passport', 'Player', Trophy],
            ['journal', 'Vault', BookOpen],
              ...
              <div className="rivalry-lobby">
                <section className={`duel-builder format-${config.mode}`} aria-labelledby="match-setup">
                  ...
                  <div className="builder-format">
                    ...
                    <p>
                      {joinView ? 'Bring the full invitation link.' : (FORMAT_COPY as any)[config.mode].rule}
                    </p>
                  </div>
                  ...
                  {joinView ? (
                    <>
                      ...
                      <p className="small-note">
                        Both screens need access to this private site. Room invitations do not grant site
                        access.
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <details className="match-options">
                        ...
                        <div className="options-grid">
                          <Picker
                            ...
                            value={config.domain}
                            onChange={(v) =>
                              change({ domain: v, topic: 'all', subtopic: 'all', region: 'all' })
                            }
                            items={[
                              ['all', 'Sports + science'],
                              ['sports', 'Sports'],
                          ...
                          <Picker
                            ...
                            items={[
                              ['all', 'Any subtopic'],
                              ...subtopics.map((v) => [v, v] as [string, string]),
                            ]}
                          />
                          <Picker
                        ...
                        <p className="small-note">
                          Coins have no cash value. Draws refund the entry. Every new room starts with 1,000
                          per player.
                        </p>
                      </details>
                      ...
                      <details className="fair-play">
                        ...
                        <p>
                          Correctness comes first, then reported answer time. Both correct within 150 ms is a
                          draw. Stay on this screen: switching away or reloading during a round cancels and
                          refunds the match.
                        </p>
              ...
              <div className="home-secondary">
                <section className="topic-spotlight">
                  <div className="minor-heading">
                    ...
                    <Button variant="ghost" onClick={() => go('collections')}>
                      All subjects
                      <ArrowRight size={16} />
                    </Button>
                  </div>
          ...
          {!room && tab === 'showroom' && (
            <section className="arena-showroom">
              ...
              {showArt ? (
                ...
                <Button onClick={() => setShowArt(true)}>Show arena artwork</Button>
              )}
              <p className="small-note">
                Drag horizontally or use the rotate controls. Switch back to the static view whenever you
                like. The scene is removed before a match starts.
              </p>
            </section>
          )}
          {!room && tab === 'collections' && (
            <Collections catalogue={catalogue} passport={player.passport} onChoose={chooseCollection} />
          )}
          {!room && tab === 'passport' && (
            <>
              <ExpeditionCase player={player} onOpen={openExpedition} />
              <Passport player={player} catalogue={catalogue} onAction={missionAction} />
            </>
          )}
          {!room && tab === 'discovery' && (
            <Discovery topic={config.topic} player={player} onBack={() => go('arena')} />
          )}
          {!room && tab === 'journal' && (
            <Journal
              journal={journal}
            ...
            <section className="room-screen">
              ...
              {['waiting', 'between'].includes(phase) && (
                <>
                  <div className="ready-panel">
                    ...
                    <p className="eyebrow">
                      {phase === 'between' ? 'A MOMENT TO LEARN' : 'THE CHALLENGE IS SET'}
                    </p>
                    <h1>
                      {phase === 'between'
                    ...
                    <div className="ready-actions">
                      ...
                      {!room.players[1] && room.seat === 0 && (
                        <Button variant="outline" onClick={() => mutate('add_bot')} disabled={busy}>
                          <Bot />
                          Add practice bot
                        </Button>
                      )}
              ...
              {['scheduled', 'playing'].includes(phase) && question && !rd.result && (
                <div className="question-area">
                  ...
                  {pending.current && !rd.answerLocked[room.seat] && !answerPending && (
                    <Button className="retry-button" variant="outline" onClick={sendPending}>
                      Retry sending this answer
                    </Button>
                  )}
                  <p className="small-note">
                    Answers lock on tap/click release or a 1–4 keypress. You get one attempt. A close result
                    can be a draw.
                  </p>
                </div>
              )}
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="settings-dialog">
          <DialogHeader>
            ...
            <DialogDescription>Preferences stay on this device. Sounds are optional.</DialogDescription>
          </DialogHeader>
          <div className="setting-row">
            <Label htmlFor="sound-setting">Game sounds</Label>
            <Switch id="sound-setting" checked={sound} onCheckedChange={setSound} />
          </div>
          <div className="setting-row">
            <Label htmlFor="theme-setting">Light theme</Label>
            <Switch id="theme-setting" checked={light} onCheckedChange={setLight} />
          </div>
          <div className="setting-row">
            ...
            <Switch
              ...
              onCheckedChange={(v) => {
                setShowArt(v);
                try {
                  localStorage.setItem('fact-duel-art', v ? 'on' : 'off');
                } catch {}
              }}
            />
          </div>
          ...
          <Button variant="outline" disabled={!sound} onClick={() => signal('win')}>
            <Volume2 />
            Preview sound
          </Button>
          <div className="settings-storage">
            <h3>Your local activity</h3>
            <p>
              Expedition progress and stamps, journal, saved question issues, passport points, missions and
              card finishes stay in this browser. Export them before resetting. Theme and sound preferences
              are kept.
            </p>
            <Button variant="outline" disabled={!player.loaded} onClick={player.exportAll}>
              Export all activity
            </Button>
            <Button
              variant="outline"
