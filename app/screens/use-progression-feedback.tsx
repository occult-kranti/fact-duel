 ...
 * screen at most, 1200 ms apart, a ceremony alone. What this file owns is the WORDS: when the budget
 * merges a burst of same-kind toasts, `mergeProgressToasts` writes the combined line
 * ("+3 quests · 140 XP") from the `meta` each toast carries.
 *
 ...
 */
const MERGE_GROUPS: Record<string, { key: string; one: string; many: string }> = {
  quest: { key: 'quest', one: 'quest', many: 'quests' },
  'quests-bonus': { key: 'quest', one: 'quest', many: 'quests' },
...
type MergeMeta = { xp?: number; one?: string; many?: string; label?: string };
 ...
 * the log entries that actually landed.
 */
function mergeProgressToasts(items: ToastInput[]): ToastInput {
  const head = items[0];
  const metas = items.map((i) => (i.meta ?? {}) as MergeMeta);
  const xp = metas.reduce((sum, m) => sum + (typeof m.xp === 'number' ? m.xp : 0), 0);
  const noun = (items.length === 1 ? metas[0].one : metas[0].many) ?? 'rewards';
  const labels = metas.map((m, i) => m.label ?? items[i].title).filter(Boolean);
  const shown = labels.slice(0, 2).join(' · ');
  const more = labels.length - 2;
  return {
    ...head,
    title: xp > 0 ? `+${items.length} ${noun} · ${xp} XP` : `+${items.length} ${noun}`,
    ...
    if (!quiet.toasts && toastQueue.current.length) {
      ...
      for (const entry of entries) {
        ...
        juice.toast({
          ...
          meta: group ? { xp: entry.xp ?? 0, one: group.one, many: group.many, label: title } : undefined,
