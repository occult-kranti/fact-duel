...
export default function ArcadeStage() {
  ...
  useEffect(() => {
    ...
    import('@/lib/arcade-scene.mjs')
      .then((m) => {
        ...
        return m.createArcadeScene(canvas.current, {
  ...
  return (
    <div className={`arcade-stage ${ready && enabled ? 'stage-live' : ''}`}>
      <img
        src="/art/rivalry-stage.webp"
        alt="Metallic lightning token and sports sphere with luminous orbital rings"
        width="1672"
        height="941"
      />
      {enabled && <canvas ref={canvas} aria-hidden="true" />}
        ...
        {enabled ? (
          <>
            ...
            <Button
              size="icon"
              variant="ghost"
              aria-label={spin ? 'Pause scene rotation' : 'Rotate scene continuously'}
              aria-pressed={spin}
            ...
            <Button
              size="icon"
              variant="ghost"
              aria-label="Return to static artwork"
          ...
          <Button variant="ghost" onClick={() => setEnabled(true)}>
            <Box />
            {failed ? 'Retry 3D view' : 'Activate 3D'}
            <ArrowRight />
          </Button>
        )}
      </div>
      {enabled && !ready && (
        <span className="stage-loading" role="status">
          Preparing the 3D view…
        </span>
      )}
      {failed && !enabled && (
        <span className="stage-loading" role="status">
          Static artwork shown. Play works normally.
        </span>
      )}
    </div>
  );
