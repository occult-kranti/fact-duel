...
/**
 * The order the Vault actually plays today, which is `dueToday` unless §3.5 seeded a deck today.
 *
 * `priority` above ranks on the LIFETIME `boldWrong` counter, so a card called-and-missed in some run
 * last month outranks the Called miss the player made ninety seconds ago, and §3.5's "Bold and Called
 * misses first" would mean "whoever has missed most often, ever, first". The stored deck is the only
 * thing in the profile that can say "these, out of THAT run, in this order", so when it is today's it
 * leads and the rest of today's queue follows.
 *
 * Only deck cards that are STILL due are played: a card answered mid-session drops out instead of
 * re-ordering everything behind it, and a deck whose cards are all answered disappears without needing
 * to be cleared. A deck from an earlier day has no claim on today's order.
 */
export function sessionOrder(journal, at = Date.now(), cap = REVIEW_CAP) {
  const seed = journal?.seed;
  const facts = journal?.facts;
  const led =
    seed && Array.isArray(seed.factIds) && date(seed.at) && dayKey(seed.at) === dayKey(at)
      ? seed.factIds.filter(
          (id) => typeof id === 'string' && facts && Object.hasOwn(facts, id) && isDue(facts[id], at),
        )
      : [];
  if (!led.length) return dueToday(journal, at, cap);
  const picked = new Set(led);
  const rest = dueToday(journal, at, Infinity).filter((id) => !picked.has(id));
  const n = Number.isFinite(cap) ? Math.max(0, Math.trunc(cap)) : led.length + rest.length;
  return [...led, ...rest].slice(0, n);
}
