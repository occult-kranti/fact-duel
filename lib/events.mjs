...
/** Domains the sanitiser accepts. Wider than what is shown, on purpose: hidden data must still parse. */
export const EVENT_DOMAINS = Object.freeze(['sports', 'science']);
/** The calendar the product shows. Every default `list` below is this, so a hidden domain never surfaces. */
export const ACTIVE_EVENTS = Object.freeze(enabledOnly(EVENTS));
 ...
 */
export function groupEvents(at = Date.now(), list = ACTIVE_EVENTS) {
  const today = todayIndex(at),
    live = [],
    upcoming = [],
    recent = [];
 ...
 */
export function monthlyModes(at = Date.now(), list = ACTIVE_EVENTS) {
...
export function activeModes(at = Date.now(), list = ACTIVE_EVENTS) {
