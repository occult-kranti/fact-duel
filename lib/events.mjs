...
// -----------------------------------------------------------------------------------------------
// Limited-time modes
const template = (id, name, tagline, when, domain, mode, duration, xpBonus, badge) =>
  Object.freeze({
    id,
    name,
    tagline,
    when,
    domain,
    duel: Object.freeze({ mode, duration }),
    xpBonus,
    badge,
  });
/**
 ...
 * `duel.duration` is a timer in seconds from the duel rules (10, 15 or 30) and `xpBonus` is a
 * multiplier in [1, 2] on the flat XP.eventMode bonus.
 */
...
export const validModeDuel = (duel) =>
  !!duel && MODES.includes(duel.mode) && [10, 15, 30].includes(duel.duration);
