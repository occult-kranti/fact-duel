 ...
 */
import { RULES, requireValue } from './server/room-engine.mjs';
...
export class MemoryRoomStore {
   ...
   */
  constructor({ clock = Date.now, yieldIO = false } = {}) {
    this.rooms = new Map();
    this.limits = new Map();
    this.now = clock;
  ...
  /** The two bounded DELETEs (LIMIT 100 each) that the create path runs. */
  async cleanup(now) {
    await this.#begin();
    let rooms = 0;
