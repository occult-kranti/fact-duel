...
export const CONFIDENCE = Object.freeze({
  steady: { name: 'Steady', correct: 2, wrong: 0 },
  bold: { name: 'Bold', correct: 3, wrong: -1 },
});
...
export function readExpeditions(value) {
  ...
  for (const route of EXPEDITIONS) {
    ...
    const best =
      first && validResult(record.best) && record.best.score >= baseline.score ? record.best : baseline;
    if (run?.cursor === 6) {
...
export function reduceExpeditions(state, action) {
  ...
  const record = state[route.key] || { run: null, first: null, best: null, last: null, completions: 0 },
    run = record.run;
  ...
  if (action.type === 'journey-next') {
    ...
    return {
      ...
      [route.key]: {
        ...
        best: !record.best || result.score > record.best.score ? result : record.best,
        last: result,
