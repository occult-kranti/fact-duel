...
export const XP = Object.freeze({
  ...
  streakCap: 7,
  levelGems: 25,
  // One entry per CONVICTION_TIERS id, including the two that pay nothing: the promotion loop reads
  // every tier it crosses, and a missing key would add `undefined` to the wallet, which nat() then
  // silently zeroes on the next load. A jump straight to Dead eye pays all four lines, 160 in total.
  convictionTierGems: Object.freeze({
    provisional: 0,
    hunch: 0,
    read: 10,
    edge: 25,
...
export const ACHIEVEMENTS = Object.freeze([
  ...
  // which §1.7 forbids outright and R2 exists to prevent: a gold award worth 300 XP and 50 gems that
...
export const COSMETICS = Object.freeze([
  ...
  cos('frame', 'called-halo', 'Called', 'A ring earned at the Edge badge.', 140, { conviction: 'edge' }),
  cos('frame', 'deadeye', 'Dead eye', 'For a Dead eye conviction badge.', 220, { conviction: 'deadeye' }),
  ...
  cos('title', 'caller', 'Caller', 'Your calls reached the Read badge.', null, { conviction: 'read' }),
  cos('title', 'sharp', 'Sharp', 'Your calls reached the Sharp badge.', null, { conviction: 'sharp' }),
  ...
  cos('banner', 'called-it', 'Called it', 'Ticker tape for the Edge badge.', 160, { conviction: 'edge' }),
  // Ungated on purpose: every other new item is unreachable for a player who never calls above Steady,
  // so without this one the gem sink does nothing for exactly the player whose surplus it exists to
  // absorb — and paying them to take the -3 tier is the one thing it must not do.
  cos('banner', 'field-notes', 'Field notes', 'Graph paper and margin scribbles.', 120),
...
/**
 * Every unlock condition on a cosmetic, met. Extracted from canEquip because the buy path has to ask
 * the same question: a priced-and-gated item was otherwise purchasable before its tier was earned.
 */
...
export function cosmeticStatus(prog, id) {
  ...
  // reduceCosmetics refuses a buy whose unlock is unmet, so an unlock-blind status here renders an
  // enabled button on the most expensive item in the catalogue that silently does nothing.
  return c.price !== null && unlockMet(prog, c) && prog.wallet.gems >= c.price ? 'buyable' : 'locked';
}
// ---------------------------------------------------------------------------------------------
// State
...
export function reduceCosmetics(prog, action, at = Date.now()) {
  ...
  if (action.type === 'cosmetic-buy') {
    // The unlock gate belongs here too: without it a priced-and-gated item is simply purchasable
    // before its tier is earned, and owning it is what canEquip checks first.
    if (
      c.price === null ||
      prog.cosmetics.owned.includes(c.id) ||
      prog.wallet.gems < c.price ||
      !unlockMet(prog, c)
    )
      return prog;
    return {
      ...
      if (fresh) {
        ...
        if (convictionIndex(to.id) > convictionIndex(convictionBest)) {
          ...
          for (const t of CONVICTION_TIERS)
            if (
              convictionIndex(t.id) > convictionIndex(convictionBest) &&
              convictionIndex(t.id) <= convictionIndex(to.id)
            ) {
              // Marked in eventBadges, which the shipped reader whitelists by shape rather than by
              // list, so the mark survives a rollback that strips `conviction` wholesale. Without it
              // an old bundle reading a new profile drops the tally and the badge while leaving the
              // wallet intact, and the next load re-walks the whole ladder and mints the 160 again.
              const mark = `conviction-${t.id}`;
              if (Object.hasOwn(eventBadges, mark)) continue;
              bonus += XP.convictionTierGems[t.id] ?? 0;
              if (XP.convictionTierGems[t.id])
                eventBadges = { ...eventBadges, [mark]: Math.max(1, Math.round(at)) };
            }
