...
export const XP = Object.freeze({
  ...
  // not out-earn a Gauntlet win. Most of the reward rides on the run score (max 18).
...
export const ACHIEVEMENTS = Object.freeze([
  ...
  ach(
    'bold-master',
    'Bold master',
    'Finish a route six for six with nothing below Bold.',
    'gold',
    (p, events) =>
      events.some(
        (e) =>
          e.kind === 'expedition-complete' &&
          e.score >= 18 &&
          (e.correct ?? 6) === 6 &&
          (e.stakes?.steady.n ?? 0) === 0,
      ),
  ),
...
export function readProgression(value) {
  ...
  next.conviction.counted = [
    ...new Set(
      (Array.isArray(cv.counted) ? cv.counted : []).filter(
        (id) =>
          typeof id === 'string' &&
  ...
  next.conviction.bestAt =
    cBest === 'provisional'
      ? null
      : Number.isSafeInteger(cv.bestAt) && cv.bestAt >= 0 && cv.bestAt <= 2000
        ? cv.bestAt
    ...
    if (route && a && a !== b) {
      ...
      if (action.type === 'journey-next' && (a.completions || 0) > (b?.completions || 0) && a.last)
        events.push({
          ...
          // The record this run has to beat to pay score XP again, read before the completion wrote
          // over it: a replayed route pays only for the improvement.
          previousBest: b?.best?.score ?? 0,
          first: !b?.first,
        });
    }
  }
