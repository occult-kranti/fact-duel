 ...
 *     show({ placement, nonceId? }): Promise<{ completed: boolean, adId?: string, reason?: string }>,
 *   };
 *
 * `nonceId` is present only in server mode (app/use-wallet.ts): the id the wallet server issued
 * for this showing, passed through untouched so an integration can tag its ad request with it.
 * The provider neither reads nor requires it.
 *
...
export class WebAdProvider {
  ...
  /** @param {{ placement: string, nonceId?: string }} request */
  async show({ placement, nonceId } = {}) {
    const h = hook();
    if (!h || typeof h.show !== 'function') return refusal('unavailable');
    if (!PLACEMENTS.includes(placement)) return refusal('failed');
    let out;
    try {
      out = await withTimeout(h.show(typeof nonceId === 'string' ? { placement, nonceId } : { placement }), SHOW_TIMEOUT_MS);
    } catch {
