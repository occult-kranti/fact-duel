...
export function reduceProfile(profile, action) {
  ...
  if (action.type === 'practice') {
    ...
    if (!profile.journal.rounds.some((r) => r.id === entry.id))
      next = { ...next, journal: { ...next.journal, rounds: [entry, ...next.journal.rounds].slice(0, 200) } };
    const prior = p.facts[f.factId];
    if (prior || Object.keys(p.facts).length < FACT_LIMIT) {
      if (!prior || !prior.recalled)
        p = {
          ...p,
          facts: {
            ...p.facts,
            [f.factId]: { topic: f.topic, opened: prior?.opened ?? false, recalled: true },
          },
        };
    }
