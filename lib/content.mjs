...
/**
 * Every topic the profile layer knows, and the domain it belongs to. The journal, passport and
 * progression sanitisers keep a fact only when its topic is a key here, so a topic a bank serves
 * must be listed before its facts can be journalled. Re-exported by lib/journal.mjs, where it lived
 * before an edition build needed to swap it (an edition aliases this whole module).
 */
