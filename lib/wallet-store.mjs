 ...
 * caller passes `at`. Below the store are the pure pieces the React hook needs and that a node
 * test can reach without a DOM: the region placeholder, the visit step (floor, then the daily
 * grant — reciprocity before any ad is offered), the floor's next due time, and the view of a
 * server balance in this same wallet shape (server mode, lib/wallet-client.ts).
 */
...
const DB_NAME = STORAGE.walletDb,
  STORE = 'wallet',
  KEY = 'wallet';
...
/**
 * The server's `wallet` reply as the reducer's wallet shape, for the screens that read
 * `wallet.wallet.coins` and `wallet.wallet.adsToday` and never need to know which mode they are in.
 * Only the three fields the server states are carried; every device-side counter (the daily key,
 * the floor stamp, the recap day) is left empty, because in server mode those grants are not
 * offered at all — the server has no endpoint for them yet — and a zero is honest where a copied
 * device value would not be. Garbage in yields the empty wallet; the result is already clean, so
 * `readWallet` hands it back by identity.
 *
 * @param {{ coins?: unknown, adsToday?: unknown, dayKey?: unknown } | null | undefined} reply
 */
export function serverWalletView(reply) {
  const r = reply && typeof reply === 'object' ? reply : {};
  return readWallet(
    Object.freeze({
      ...emptyWallet(),
      coins: Number.isSafeInteger(r.coins) && r.coins >= 0 ? r.coins : 0,
      adsToday: Number.isSafeInteger(r.adsToday) && r.adsToday >= 0 ? r.adsToday : 0,
