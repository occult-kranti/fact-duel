...
//
// Two counts per mode, both of them things the database can prove:
...
//    Same predicate as `liveCount`, so the number under a card and the number inside a search agree.
//  - `inGame`: rooms whose state says they are mid-match — phase scheduled, playing or between —
//    and whose `expires_at` has not passed. `json_extract` reads the phase and the mode out of the
//    JSON `state` column, so no room has to be parsed in JS and no second table is needed.
//
...
export async function presence(db, { now } = {}) {
  ...
  const playing = await d
    .prepare(
      `SELECT json_extract(state,'$.config.mode') AS mode, COUNT(*) AS n FROM rooms
   WHERE expires_at>=? AND json_extract(state,'$.phase') IN (${IN_GAME_LIST}) GROUP BY mode`,
    )
    .bind(at)
    .all();
