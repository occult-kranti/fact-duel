...
export const OPS = Object.freeze({
  /** The only sweep kind today. Later milestones add 'reconcile'; `health` reports on all of them. */
  kinds: Object.freeze(['sweep']),
...
export async function sweep({ store, now = Date.now(), clock = Date.now } = {}) {
  ...
  try {
    ...
    const detail = 'cleanup: rooms, admission_limits';
