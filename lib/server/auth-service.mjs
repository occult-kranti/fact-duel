...
/** The providers that PROVED an address: a link was clicked, or Google vouched for it. */
export const PROVED_PROVIDERS = Object.freeze(['email', 'google']);
const isProved = (provider) => PROVED_PROVIDERS.includes(provider);

/**
 * What the client may know about who it is.
 *
 * `signedIn` is the honest one: it is true only when this request carries a session AND the
 * principal has an identity somebody PROVED (a clicked link or Google). A `claimed-email` row is
 * an address typed into the profile gate and checked by nobody, so it can never make this true —
 * otherwise the account panel would say "Signed in as <address>" about a stranger's address. The
 * claim is reported instead under `claimed`, which is what Settings prints beside "not verified".
 *
 * `session` says only that the cookie resolved, which is what profile sync turns on for: a claim
 * does get a session (the card follows the claim on this device), it just is not a sign-in.
 *
 * `email` prefers a proved address over a claimed one, so verifying by link later replaces what
 * was typed on day one rather than losing to it on age. `providers` lists every method on the
 * principal, oldest first.
 */
export async function whoami(db, { principalId = null, kind = 'anonymous' } = {}) {
  if (!principalId)
    return Object.freeze({ signedIn: false, session: false, principalId: null, email: null, claimed: null, providers: [] });
  ...
  const proved = results.filter((r) => isProved(r.provider));
  const claimedEmail = results.find((r) => !isProved(r.provider) && r.email)?.email ?? null;
  const email = proved.find((r) => r.email)?.email ?? claimedEmail;
  return Object.freeze({
    signedIn: session && proved.length > 0,
    session,
    principalId,
    email: session ? email : null,
    claimed: session && claimedEmail ? Object.freeze({ email: claimedEmail }) : null,
    providers: session ? results.map((r) => r.provider) : [],
  });
}
 ...
 * The class is written with escapes rather than the bytes themselves, so the file stays text and
 * a repo-wide grep (the release gate in tests/offline-copy.test.mjs is one) can still read it.
 */
export function readDisplayName(value) {
  if (typeof value !== 'string') return null;
  const name = value.replace(/[\0-\x1f\x7f]/g, ' ').replace(/\s+/g, ' ').trim();
 ...
 * benefit of the claim: a name, a card that syncs, and an address to send the link to later. It is
 * NOT a sign-in, and `whoami` above says so: `signedIn` stays false and the address is reported
 * under `claimed`, so no screen can call an unproved address an account.
 */
