 ...
 *
 *   POST /api/auth  { action: 'whoami' | 'quick-profile' | 'request-link' | 'google' | 'signout', ... }
...
export async function handleAuthRequest(request, env, { now = Date.now, fetchImpl, mailer, keyCache } = {}) {
  try {
    ...
    if (action === 'quick-profile') {
      // The profile gate's claim: a name and an UNVERIFIED address (`quickProfile`'s comment says
      // what that does and does not buy). A first landing can arrive before the wallet has minted
      // a guest id, so an anonymous request gets a fresh principal here, as an unknown sign-in
      // would; the session cookie below is what the device is known by from then on.
      const result = await quickProfile(db, {
        principalId: who.principalId ?? newPrincipalId(),
        name: body?.name,
        email: body?.email,
        now: at,
      });
      return ok(
        { ok: true, principalId: result.principalId, email: result.email },
