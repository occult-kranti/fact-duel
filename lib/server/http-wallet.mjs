 ...
 */
import { D1WalletStore, issueAdNonce, readWallet, redeemAdNonce, readRegion } from './wallet-service.mjs';
...
export async function handleWalletRequest(request, env) {
  try {
    ...
    if (action === 'wallet') return new Response(JSON.stringify(await readWallet(common)), { headers });
